...
/**
 * Handles download requests for files stored by File Upload fields in google cloud storage
 *
 * Class GC_GF_Download from gravityforms/includes/class-gf-download.php
 */
class GC_GF_Download {
  ...
  public static function maybe_process() {
    if ( isset( $_GET['gc-gf-download'] ) ) {

      $file     = $_GET['gc-gf-download'];
  ...
  private static function deliver( $form_id, $file ) {
    // WP core upload dir
    $upload_root_dir = Proud\Gform\get_upload_root_dir();
    // Gform upload dir
    $path  = str_replace( $upload_root_dir, '', GFFormsModel::get_upload_path( $form_id ) );
    $file_path = ud_get_stateless_media()->get( 'sm.root_dir' ) . trailingslashit( $path ) . $file;
    ...
    if ( $media = $client->media_exists( $file_path ) ) {
      GFCommon::log_debug( __METHOD__ . '(): File exists. Starting delivery.' );

      $file_url = trailingslashit('https://storage.googleapis.com/' . ud_get_stateless_media()->get( 'sm.bucket' )) . $file_path;
      ...
      self::readfile_chunked( $file_url );
      die();
    } else {
