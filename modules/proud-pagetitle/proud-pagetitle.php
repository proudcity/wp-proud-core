...
function proud_pagetitle_get_duplicates($title, $id) {
   global $wpdb;
   $title_exists = $wpdb->get_results( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_status = 'publish'", sanitize_text_field( $title ) ) );
...
function proud_pagetitle_callback() {
    ...
    if( isset( $_POST['_wpnonce'] ) && wp_verify_nonce($_POST['_wpnonce'], 'proud_pagetitle_check') ) {
        if( isset( $_POST['title'], $_POST['post_id'] ) &&  proud_pagetitle_get_duplicates( $_POST['title'], $_POST['post_id'] ) ) {
            wp_send_json(array(
                'duplicate_exists' => 1,
